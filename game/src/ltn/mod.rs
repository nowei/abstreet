        ...
        for i in [current.src_i, current.dst_i] {
            for next in &map.get_i(i).roads {
                ...
                if let Some(filter) = modal_filters.intersections.get(&i) {
                    if !filter.allows_turn(map, current.id, *next) {
                        continue;
                    }
                }
...
impl DiagonalFilter {
    ...
    fn geometry(&self, app: &App) -> PolyLine {
        ...
        // Does order matter? As long as they're not both right or both left...
    ...
    fn allows_turn(&self, map: &Map, from: RoadID, to: RoadID) -> bool {
        let mut roads = map.get_i(self.i).get_roads_sorted_by_incoming_angle(map);
        // Make self.r1 be the first entry
        while roads[0] != self.r1 {
            roads.rotate_right(1);
        }

        let mut group1 = vec![roads.remove(0)];
        while roads[0] != self.r2 {
            group1.push(roads.remove(0));
        }
        // group2 is now 'roads'

        // Now we just see if the two requested roads are in the same group
        group1.contains(&from) == group1.contains(&to)
    }
}
